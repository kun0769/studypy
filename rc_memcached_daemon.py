...
class Process(object):
    ...
    args = {'PORT':11211,'USER':'memcached','MAXCONN':1024,'CACHESIZE':64,'OPTIONS':''}  #定义开始默认参数

    def __init__(self, name, program, workdir):  #实例化自动加载该方法 name命令名字 program 执行文件路径
        self.name = name                               #workdir PID目录
        self.program = program                         #类的属性接受参数
        self.workdir = workdir

#'''  读取memcached配置文件'''
    def readConf(self,f):                         #读取memcached配置文件 /ect/sysconfig/memcached内容
        with open(f) as fd:                       #并以该文件内容为最新的参数以字典形式返回
            lines = fd.readlines()
            return dict([i.strip().replace('"','').split('=') for i in lines]) #strip去空格 replace去” split去=

#''''  替换默认参数'''
    def replaceArgs(self):
        conf = self.readConf('/etc/sysconfig/memcached')  #/ect/sysconfig/memcached内的参数替换掉默认参数并返回
        if 'PORT' in conf: 
        ...
        options = ['-p',self.args['PORT'],
                   '-u',self.args['USER'],
                   '-m',self.args['MAXCONN'],
                   '-c',self.args['CACHESIZE'] ]
        os.system('chown %s %s' %(self.args['USER'], self.workdir))  #更改var/tmp/memcached权限 来创建该目录
        return options
...
#'''  定义start方法 '''
    def start(self):
        ...
        cmd = [self.program] + self.replaceArgs() + ['-d','-P',self.pidFile()]
        print cmd                                    #打印出命令内容
        p = Popen(cmd, stdout=PIPE)                  #命令通过管道输出
        #self.pid = p.pid                            #得到pid数值并定义对象公有属性 让weiterPid方法可以调用 
        #self.writePid()                             #调用writePid方法 -P会自动把pid写进文件中因此不需要这两句
...
def main():       ##定义一个主函数
    ...
    pm = Process(name = name, program = prog, workdir = wd)   ##实例化对象 并传参
